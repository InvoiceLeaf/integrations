...
export const buildReminderTriggeredMessage: IntegrationHandler<ReminderTriggeredInput | Record<string, unknown>, ReminderTriggeredOutput, TelegramBotConfig> = async (input, context) => {
  try {
    context.logger.info('Building Telegram payload for reminder.triggered', {
      input,
    });

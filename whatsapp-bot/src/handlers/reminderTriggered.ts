...
export const buildReminderTriggeredMessage: IntegrationHandler<ReminderTriggeredInput | Record<string, unknown>, ReminderTriggeredOutput, WhatsAppBotConfig> = async (input, context) => {
  try {
    context.logger.info('Building Whatsapp payload for reminder.triggered', {
      input,
    });

...
import type { StripeIntegrationConfig } from '../types.js';

export function createMockContext(
  configOverrides: Partial<StripeIntegrationConfig> = {}
): IntegrationContext<StripeIntegrationConfig> {
  return {
    spaceId: 'space-1',
    userId: 'user-1',
    ...
    credentials: {
      ...
      getApiKey: vi.fn().mockResolvedValue('rk_test_123'),
      refreshToken: vi.fn().mockResolvedValue('refreshed-token'),
      getConnectionInfo: vi.fn().mockResolvedValue({ connected: true, provider: 'stripe' }),
    },
...
export function stripeList<T>(data: T[], hasMore = false): object {
  return jsonResponse({ object: 'list', data, has_more: hasMore });
}
